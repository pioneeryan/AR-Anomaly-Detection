...
class AR:
    ...
    def prediction_error(self, original, time):
        n = len(original)
        lst = []
        s = 0
        psi = self.psi(time)
...
# This function returns the position of the anomalies in the list of test data.
    def anomaly_rate(self, original, test):  # Tested
        ...
        forecast_error = self.prediction_error(original, time)
...
def anomaly_detection_score(original, test, candidate_num=7, used_num=3, threshold=2, models=[], if_return_score=False):
    if not models:
    ...
    for m in models:
        now = m.anomaly_rate(original, test)
        for i in range(time):
    ...
    for i in range(time):
        if anomaly_score[i] > standard:
            anomaly.append(i)
    anomaly_score = list(map(lambda x: x / len(models), anomaly_score))
    anomaly_score_of_anomaly = []
    for i in anomaly:
        anomaly_score_of_anomaly.append(anomaly_score[i])
    if if_return_score:
        return anomaly, anomaly_score_of_anomaly
    else:
        return anomaly


def anomaly_detection_more_than_two(original, test):
    models = model_estimator(original)
    time = len(test)
    anomaly_times = []
    anomaly_1 = models[0].anomaly_rate(original, test)
    anomaly_2 = models[1].anomaly_rate(original, test)
    anomaly_3 = models[2].anomaly_rate(original, test)
    for i in range(time):
        score = 0
        if anomaly_1[i] > 2:
            score += 1
...
def synthetic_demo():
    ...
    for x in c:
        ...
        print('Result',x.anomaly(attempt[:training_t], attempt[training_t:]))
        plt.plot(attempt[training_t - 20:training_t] + x.prediction(attempt[:training_t], 20))
...
def flight_data_stair_demo_1():
    ...
    for demo_model in model_estimator(demo_training):
        ...
        print('Model anomaly rate', demo_model.anomaly_rate(demo_training, demo_test))
    b_0 = anomaly_detection_score(demo_training, demo_test)
    print('Result', b_0)
    plt.plot(demo_test)
    for demo_model in model_estimator(demo_training):
        plt.plot(demo_model.prediction(demo_training, 10))
    plt.show()
    return
...
def flight_data_stair_demo_2_differenced():
    ...
    demo_test = para6_resample_diff[35:]
    for demo_model in model_estimator(demo_training):
        print('Model order', demo_model.order)
        print('Model coefficient', demo_model.phi)
        print('Model sigma', demo_model.noise_variance)
        print('Model anomaly rate', demo_model.anomaly_rate(demo_training, demo_test))
    b_0 = anomaly_detection_more_than_two(demo_training, demo_test)
    print('Result', b_0)
    plt.plot(para6_resample_diff)
    plt.show()
    return
